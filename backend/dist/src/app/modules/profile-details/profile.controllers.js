...
const reportProfile_model_1 = require("./report-profile/reportProfile.model");
...
const profileMatch_service_1 = require("../../services/profileMatch.service");
...
const getProfiles = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // ==========================================
        // Calculate mutual match percentage
        // ==========================================
        const loggedInPreference = yield partnerPreference_model_1.PartnerPreference.findOne({
            profileId: loggedInProfile._id,
            isDeleted: false,
        });
        let profilesWithMatchPercentage = profiles.map((profile) => (Object.assign(Object.assign({}, profile.toObject()), { match: 0 })));
        if (loggedInPreference && profiles.length > 0) {
            ...
            const preferenceMap = new Map(partnerPreferences.map((preference) => [
                preference.profileId.toString(),
                preference,
            ]));
            profilesWithMatchPercentage =
                profiles.map((profile) => {
                    const otherPreference = preferenceMap.get(profile._id.toString());
                    const matchPercentage = otherPreference
                        ? (0, profileMatch_service_1.calculateMatchPercentage)(loggedInProfile, loggedInPreference, profile, otherPreference)
                        : 0;
                    return Object.assign(Object.assign({}, profile.toObject()), { match: matchPercentage });
                });
        }
        profiles.sort((a, b) => {
            var _a, _b;
        ...
        return res.status(200).json({
            success: true,
            total: profilesWithMatchPercentage.length,
            data: profilesWithMatchPercentage,
        });
    }
    catch (error) {
...
const deleteProfile = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // 2. Delete Profile
        yield profile_model_1.Profile.deleteOne({ userId }, { session });
        // 3. Delete Interests
        ...
        // 6. Delete Partner Preference
        yield partnerPreference_model_1.PartnerPreference.deleteMany({
            userId,
        }, { session });
        // 7. Delete Account Settings
        yield accountSettings_model_1.AccountSettings.deleteMany({
            userId,
        }, { session });
        // 8. Delete Profile Visits
        yield profileVisits_model_1.default.deleteMany({
            $or: [
                { visitorId: userId },
                { visitedUserId: userId },
            ],
        }, { session });
        // 9. Delete Profile Reports
        yield reportProfile_model_1.ProfileReport.deleteMany({
            $or: [
                { reporterId: userId },
