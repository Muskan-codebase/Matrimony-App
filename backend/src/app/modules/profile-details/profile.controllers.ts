...
import { ProfileReport } from "./report-profile/reportProfile.model";
...
import { calculateMatchPercentage } from "../../services/profileMatch.service";
...
export const getProfiles = async (req: Request, res: Response) => {
    try {
        ...
        const loggedInPreference = await PartnerPreference.findOne({
            profileId: loggedInProfile._id,
            isDeleted: false,
        });

        let profilesWithMatchPercentage = profiles.map((profile) => ({
            ...profile.toObject(),
            match: 0,
        }));
        ...
        if (loggedInPreference && profiles.length > 0) {
            ...
            const preferenceMap = new Map(
                partnerPreferences.map((preference) => [
...
export const deleteProfile = async (req: Request, res: Response) => {
    ...
    try {
        ...
        await ProfileReport.deleteMany(
            {
                $or: [
                    { reporterId: userId },
