 ...
 * /v1/api/call/token:
 *   post:
 *     summary: Get a ZegoCloud call token
 *     description: |
 *       Checks the authenticated user's package callLimit / dailyCallLimit
 *       (via checkCallLimit) and, if allowed, returns a ZegoCloud token +
 *       deterministic roomId to join for a voice/video call with the given
 *       receiver. Returns 403 if the caller's package limit is exceeded.
 *     tags:
 *       - Voice Call
 *     security:
 *       - bearerAuth: []
 ...
 *       403:
 *         description: Call limit reached for the current package.
 *       401:
 *         description: Unauthorized.
 */
router.post('/token', authenticate, getCallTokenController);
/**
 * @swagger
 * /v1/api/call/update:
 *   post:
 ...
 *         description: Internal server error.
 */
router.get('/history', authenticate, getCallsController);
